...
u8 mb_toolbar[9];
u8 mb_toolbar_params[9];
u8 mb_toolbox[18 * 5];
u8 mb_toolbox_params[18 * 5];
...
Gfx *get_button_tex(u32 buttonId, u32 objIndex) {
    struct MBButtonUIType *button = &mb_ui_buttons[buttonId];
    if (button->placeMode != MB_PM_TILE) {
        u32 id;
        if (button->multiObj) {
        ...
        return mb_object_type_list[id].btn;
    }
    if (buttonId == MB_BUTTON_BLANK) return mat_b_btn_blank;
    return mb_terrain_info_list[button->id].button;
}
...
char *get_button_str(u32 buttonId) {
    ...
    if (button->placeMode != MB_PM_TILE) {
        if (button->multiObj) {
            ...
            u32 id = button->id;
            return mb_object_type_list[id].name;
        }
    }
    return mb_terrain_info_list[button->id].name;
}
...
int sample_block(int index) {
    ...
    for (int i = 0; i < mb_object_count; i++) {
        struct MBObject *obj = &mb_object_data[i];
        if ((obj->x == mb_cursor_pos[0]) && (obj->y == mb_cursor_pos[1]) && (obj->z == mb_cursor_pos[2])) {
            isObject = TRUE;
            targetId = obj->type;
            targetBparam = obj->bparam;
            mb_rot_selection = obj->rot;
            break;
        }
    }
    // Look at tile array
    if (!isObject) {
        struct MBGridObject *tile = get_grid_tile(mb_cursor_pos);
        if (tile->type == TILE_TYPE_EMPTY) {
            return FALSE;
        }
        targetId = tile->type;
        targetBparam = tile->mat;
        mb_rot_selection = tile->rot;
        mb_upsidedown_tile = FALSE;
        if (targetId < TILE_END_OF_FLIPPABLE && targetId & 1) {
            targetId &= ~1;
            mb_upsidedown_tile = TRUE;
        }
    }

    // Find relevant button
    for (int i = 0; i < MB_BUTTON_COUNT; i++) {
        struct MBButtonUIType *buttonInfo = &mb_ui_buttons[i];
        if (isObject ^ (buttonInfo->placeMode == MB_PM_OBJ)) {
            continue;
        }
        ...
        if (isObject && buttonInfo->multiObj) {
            ...
            if (!isObject) mb_mat_selection = targetBparam;
...
int do_sample_buffer_check(void) {
    ...
    if (bothPressed && mb_place_mode != MB_PM_ACTION) {
        doSample = TRUE;
    }
...
void component_toolbar_loop(MenuComponent *m, s16 x, s16 y) {
    ...
    struct MBButtonUIType *buttonInfo = &mb_ui_buttons[id];
    ...
    char *yellowStr = NULL;
    mb_place_mode = buttonInfo->placeMode;

    // Handle updating editor values and switching types with Dpad
    int dir = 0;
    int toolbarActive = ACTIVE && mb_menu_state == MB_MAKE_MAIN;
    if (toolbarActive) {
    ...
    if (mb_place_mode != MB_PM_TILE) {
        ...
        if (param != 0 && dir) {
            ...
            mb_toolbar_params[toolbar->index] = curButton->buttonParam;
            menu_play_move_sound();
        }
        // Set mb_id_selection and the string to display
        if (buttonInfo->multiObj) {
            mb_id_selection = buttonInfo->idList[curButton->buttonParam];
            yellowStr = mb_object_type_list[mb_id_selection].name;
        } else {
            mb_id_selection = buttonInfo->id;
            if (buttonInfo->names) {
                yellowStr = buttonInfo->names[curButton->buttonParam];
            }
            mb_param_selection = curButton->buttonParam;
        }
    } else {
        mb_id_selection = buttonInfo->id;
        // Tiles with materials
        if (mb_terrain_info_list[mb_id_selection].terrain) {
            // Tile flipping
            int tileFlippable = (mb_id_selection < TILE_END_OF_FLIPPABLE) && (mb_place_mode == MB_PM_TILE);
            if (tileFlippable) {
                if (toolbarActive && gPlayer1Controller->buttonPressed & U_JPAD) {
                    play_sound(SOUND_ACTION_SIDE_FLIP_UNK, gGlobalSoundSource);
                    mb_upsidedown_tile ^= 1;
                }
                if (mb_upsidedown_tile) {
                    mb_id_selection = (mb_id_selection & ~1) | 1;
            ...
            // Material switching
            if (dir) {
                mb_mat_selection = (mb_mat_selection + dir + NUM_MATERIALS_PER_THEME) % NUM_MATERIALS_PER_THEME;
                menu_play_move_sound();
            }
            yellowStr = TILE_MATDEF(mb_mat_selection).name;
        }
    }
...
void create_toolbar(void) {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        button->buttonID = mb_toolbar[i];
        button->buttonParam = mb_toolbar_params[i];
...
void toolbox_render_button(Selector2DComponent *s, s16 x, s16 y, u8 column, u8 row, int selected) {
    ...
    if (selected) {
        ...
        sSelectedX = x;
        sSelectedY = y;
        if (mb_ui_buttons[mb_toolbox[index]].multiObj) {
            int maxParam = mb_ui_buttons[mb_toolbox[index]].paramCount;
            int dir = get_input(MENU_INPUT_DPAD, DIR_HORIZONTAL);
            mb_toolbox_params[index] = (mb_toolbox_params[index] + maxParam + dir) % maxParam;
    ...
    render_button(mb_toolbox[index], mb_toolbox_params[index], selected, x, y);
}

void toolbox_select_button(Selector2DComponent *s, u8 column, u8 row) {
    if (mb_toolbox[s->index] == MB_BUTTON_BLANK) return;
    play_sound(SOUND_ACTION_BRUSH_HAIR, gGlobalSoundSource);

    ListComponent *toolbar = get_child(gToolbar);
    create_animated_button(sSelectedX, sSelectedY + 3, toolbar->index, mb_toolbox[s->index], mb_toolbox_params[s->index]);
}

void toolbox_render_text(MenuComponent *m, s16 x, s16 y) {
    FrameComponent *f = (FrameComponent *)m;
    Selector2DComponent *box = get_parent(f);
    if (mb_toolbox[box->index] == MB_BUTTON_BLANK) return;

    x = sSelectedX + 18;
    y = sSelectedY - 3;

    // Render name for selected button
    char *buttonName = get_button_str(mb_toolbox[box->index]);
    struct MBButtonUIType *buttonInfo = &mb_ui_buttons[mb_toolbox[box->index]];
    ...
    if (isMulti) {
        u32 objId = buttonInfo->idList[mb_toolbox_params[box->index]];
        sprintf(stringBuf, "< %s >", mb_object_type_list[objId].name);
