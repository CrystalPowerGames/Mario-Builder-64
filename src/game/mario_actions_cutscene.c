 ...
 * 2: Mario must not be riding a shell or be invulnerable.
...
s32 act_reading_sign(struct MarioState *m) {
    ...
    switch (m->actionState) {
        ...
        case ACT_STATE_READING_SIGN_TURN_TOWARD_SIGN:
            ...
            // create the text box
            if (m->actionTimer++ == 10) {
                create_dialog_box("Arthur hasn't done these\nyet either.");
...
s32 act_bbh_enter_jump(struct MarioState *m) {
    play_mario_action_sound(
        m, m->flags & METAL_SOUND_FLAG ? SOUND_ACTION_METAL_JUMP : SOUND_ACTION_TERRAIN_JUMP, 1);
...
s32 act_squished(struct MarioState *m) {
    ...
    // steep floor
    if (m->floor != NULL && m->floorNormal[1] < 0.5f) {
    ...
    if (m->ceil != NULL) {
        Vec3f normal;
        get_surface_normal(normal, m->ceil);
        if (-0.5f < normal[1]) {
            surfAngle = m->ceilYaw;
            underSteepSurf = TRUE;
        }
