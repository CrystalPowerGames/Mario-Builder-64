...
//OTHER LB CODE
    ...
    return drop_and_set_mario_action(m, ACT_LAVA_BOOST, (SURFACE_IS_BURNING_SMOKE(m->wall->type) ? 1 : 3));
}
...
void update_flying(struct MarioState *m) {
    ...
    m->forwardVel -= 2.4f * ((f32) m->faceAngle[0] / 0x4000) + 0.1f;
    m->forwardVel -= 0.7f * (1.0f - coss(m->angleVel[1]));
...
s32 act_warp_pipe(struct MarioState *m) {
...
s32 act_butt_slide_air(struct MarioState *m) {
    ...
    switch (perform_air_step(m, AIR_STEP_CHECK_NONE)) {
        case AIR_STEP_LANDED:
            if (m->actionState == 0 && m->vel[1] < 0.0f && m->floorNormal[1] >= COS10) {
...
s32 act_hold_butt_slide_air(struct MarioState *m) {
    ...
    switch (perform_air_step(m, AIR_STEP_CHECK_NONE)) {
        case AIR_STEP_LANDED:
            if (m->actionState == ACT_STATE_BUTT_SLIDE_AIR_SMALL_BOUNCE && m->vel[1] < 0.0f && m->floorNormal[1] >= COS10) {
                m->vel[1] = -m->vel[1] / 2.0f;
                m->actionState = 1;
            } else {
    ...
    switch (perform_air_step(m, 0)) {
        ...
        case AIR_STEP_HIT_WALL:
            if (m->wall != NULL) {
                ...
                play_sound((m->flags & METAL_SOUND_FLAG) ? SOUND_ACTION_METAL_BONK
                                                        : SOUND_ACTION_BONK,
                           m->marioObj->header.gfx.cameraToObject);
...
s32 mario_execute_airborne_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_WALL_STICK:           cancel = act_wall_stick(m);           break;
        case ACT_WARP_PIPE:            cancel = act_warp_pipe(m);
    }
    /* clang-format on */
